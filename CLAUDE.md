...
Before scraping, the bot short-circuits to decision `native_feed` (→ workflow
comments with the feed URL, labels `native-feed`, closes the issue; no PR, no
scraping) in three cases: the URL matches a known native-feed pattern
(`native_feed_candidate` — GitHub releases/tags/commits + user/org `.atom`,
verified pre-fetch); the fetched page **is itself a feed** (`_is_feed_payload`);
or the page **advertises a feed** via `<link rel="alternate">` autodiscovery
(`discover_feed_in_html`, verified by `looks_like_feed`). The pattern check is
pre-fetch; the other two run on the fetched HTML.
...
The "Available RSS Feeds" table in `README.md` is **generated**, not hand-edited.
`.github/scripts/update_readme.py` rewrites whatever is between the
`<!-- FEEDS:START -->` / `<!-- FEEDS:END -->` markers with a row per enabled feed
in `feeds.yaml`. `.github/workflows/update-readme.yml` runs it on pushes to `main`
that change `feeds.yaml` (e.g. a merged feed-request PR) and commits the result
with `[skip ci]` (so it doesn't retrigger build/deploy). To change the table,
edit `feeds.yaml` or the script — not the markers' contents. (The Pages landing
page `docs/index.html` is separately auto-generated by `generate.py`.)
